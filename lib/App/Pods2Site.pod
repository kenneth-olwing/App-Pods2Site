...
However, it also allows you to search arbitrary locations for pods, as well as cherrypick among the found pods and just
include ones of interest to you.
...
Taking a cue from the ActivePerl way of collecting the pods, pods are divided in 4 categories. The rules for doing this are fairly
simpleminded and heuristic though, so occasionally you may find a module that doesn't really conform to the categorization. Sorry.
