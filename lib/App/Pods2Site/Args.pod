...
This style uses a simple frames layout, but the TOC is implemented with the HTML5 'details' element in order
to create a simple open/close tree control.

=item * basicframes-simple-toc

This style is very similar to the above, but the TOC is rendered as a static long indented list. 
...
and used in the generated HTML files, and also by passing the '--css' option to the pod2html command.
